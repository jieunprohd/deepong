...
> **딥워크는 친구와 편히 소통하면서도 서로의 집중을 지켜주는 데스크톱 메신저다.**
...
딥워크의 핵심 도메인은 **"비동기 소통의 조율 (Asynchronous Communication Orchestration)"** 입니다.

기존 메신저들은 "메시지 = 알림"으로 간주하지만, 딥워크는 이 둘을 별개의 도메인 개념으로 분리합니다:
...
딥워크는 **6개 바운디드 컨텍스트**로 구성됩니다. 각 컨텍스트는 독립된 ubiquitous language와 모델을 가지며, 명시적 계약으로만 통신합니다.
...
**Identity와 Relationship 분리**: User 자체 관리는 표준적(OAuth, JWT)이지만 친구 관계와 커뮤니케이션 규범은 딥워크만의 독자 개념입니다. Identity가 바뀌어도 Relationship 모델은 그대로여야 합니다.
